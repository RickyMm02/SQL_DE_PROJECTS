...
UNION
SELECT UNNEST([1,1,3]); --2,1,3

SELECT UNNEST([1,1,1,2])
UNION ALL
SELECT UNNEST([1,1,3]); --1,1,1,2,1,1,3

-- INTERSECT Operator
SELECT UNNEST([1,1,1,2])
INTERSECT
SELECT UNNEST([1,1,3]); --1

SELECT UNNEST([1,1,1,2])
INTERSECT ALL
SELECT UNNEST([1,1,3]); --1,1

-- EXCEPT Operator

SELECT UNNEST([1,1,1,2])
EXCEPT
SELECT UNNEST([1,1,3]); --2

SELECT UNNEST([1,1,1,2])
EXCEPT ALL
SELECT UNNEST([1,1,3]); --1,2
...
-- Which job postings from 2023 remain after subtracting matching 2024 posting, one-for-one?
